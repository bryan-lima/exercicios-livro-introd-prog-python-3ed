...
def show_data(name, phone, birthday, email):
...
def new():
    ...
    email = ask_email()
    phoneBook.append([name, phone, birthday, email])
...
def update():
    ...
    if result is not None:
        ...
        birthday = phoneBook[result][2]
        email = phoneBook[result][3]
        print('Encontrado:')
        show_data(name, phone, birthday, email)
        name = ask_name(name)
        phone = ask_phone(phone)
        birthday = ask_birthday(birthday)
        email = ask_email(email)
        if confirm('alteração'):
            phoneBook[result] = [name, phone, birthday, email]
            changedPhoneBook = True
        ...
        try:
            with open(file_name, 'w', encoding='utf-8') as file:
                for register in phoneBook:
                    file.write(f'{register[0]}#{register[1]}#{register[2]}#{register[3]}\n')
